...
blz_repair =
{
	...
	if(_damage > 0 && triggerActivated _trg) then
	{
		...
		{
			titleText ["Reparieren...","PLAIN DOWN"];
			sleep sleeptimer_s;
	...
	if(_fuel < 1 && triggerActivated _trg) then
		{
			...
			while {_fuel < 1 && triggerActivated _trg} do
				{
					titleText ["Auftanken...","PLAIN DOWN"];
					sleep sleeptimer_s;
	...
	if(count _weapon > 1 && triggerActivated _trg) then
		{
				...
				{
					titleText ["Aufmunitionieren...","PLAIN DOWN"];
					sleep sleeptimer_m;
...
/*______________________________________________________________________________________________*/
	...
	if (_count_turrets > 0) Then {
		...
		{
			titleText ["Geschütze aufmunitionieren...","PLAIN DOWN"];
			sleep sleeptimer_m;
	...
	if (_count_turrets_cmd > 0) Then {
		...
		{
			titleText ["Komandantenplatz aufmunitionieren...","PLAIN DOWN"];
			sleep sleeptimer_m;
...
into =
{
	...
	sleep sleeptimer_s;
	titleText ["Fahrzeugdiagnose wird durchgeführt...", "PLAIN DOWN"];
...
{
	sleep sleeptimer_s;
	titleText ["Fahrzeugservice abgeschlossen", "PLAIN DOWN"],
...
if (triggerActivated _trg && _object != player && player == driver _object) then {
	sleep sleeptimer_s;
			...
			if(triggerActivated _trg) Then {
				SS = driver _object addAction ["<t color='#f69619'>Fahrzeugservice</t>", {call blz_ss}, [_object,_trg,_trg_pos,_trg_agl,_trg_txt]];

				if(_trg_txt == "blz_turn") Then
					{
						SS_turn = driver _object addAction ["<t color='#f69619'>Fahrzeugservice w/Turn</t>", {call blz_ss_turn}, [_object,_trg,_trg_pos,_trg_agl,_trg_txt]];
					};
